                ...
                <?php if (!empty($reviews)) { ?>
                                ...
                                <div class="d-flex align-items-center">
                                    <form action="?target=review&action=likeReview&id=<?= $movie['id'] ?>" method="POST" class="m-0 p-0 me-3">
                                        ...
                                        <?php if (isset($_SESSION['user'])) { ?>
                                            <button type="submit" name="like_review" class="btn btn-sm btn-outline-light like-btn">
                                                <i class="bi bi-hand-thumbs-up"></i> Like
                                            </button>
                                        <?php } ?>
                                    </form>
                                    <span class="like-count text-warning me-auto"><?= $review['like_count'] ?> likes</span> 
                    ...
                    <ul class="pagination justify-content-center">
                        ...
                        <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
                            <a class="page-link" href="?target=movie&action=showMovie&id=<?= $movieId ?>&page=<?= $page - 1 ?>" tabindex="-1">« Previous</a>
                        </li>

                        <!-- Номера на страниците -->
                        <?php for ($i = 1; $i <= $totalPages; $i++) { ?>
                            <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                <a class="page-link" href="?target=movie&action=showMovie&id=<?= $movieId ?>&page=<?= $i ?>"><?= $i ?></a>
                            </li>
                        <?php } ?>
