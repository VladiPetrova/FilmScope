...
class MovieController {
    ...
    public function addMovie() {
        ...
        if (isset($_POST["add_movie"])) {
            ...
            // Ако няма грешка от качването
            if (!$error) {
                // Създаваме филм в базата
                $result = $movieModel->create($movieTitle, $movieDescription, $movieYear, $posterPath, $genre, $actors, $_SESSION['user']);
                if ($result === true) {
                    // Успешно добавяне - показваме страница за успех и прекратяваме метода
                    require 'View/movies/success_page.php';
                    return;
                } else {
                    //Изписва ако е станала някаква грешка
                    switch ($result) {
    ...
    public function showMovie() {
        ...
        // Проверка дали е подаден параметър id, ако не – показва грешка
        if (!isset($_GET['id'])) {
        ...
        // Вземаме рецензиите за текущата страница, броя на всички рецензии и общия брой страници
    ...
    public function searchResults() {
        if (isset($_POST['query'])) {
            ...
            if ($query == '') {
                header("Location: index.php?target=movie&action=searchResults");
                die();
            }
